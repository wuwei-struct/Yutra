...
function success(contextPatch: Record<string, unknown>): ActionHandlerResult {
  return {
    ok: true,
    output: contextPatch,
    contextPatch,
    meta: { mode: "deterministic_fixture", networkAccess: false }
  };
}
...
export const EXPLICIT_DEMO_ACTION_REGISTRY: ActionRegistry = {
  ...
  notify_result: handler(() => ({ notification_mode: "demo_mock" })),
