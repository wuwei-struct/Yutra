...
describe("P6-07A approval-decision second archetype conformance", () => {
  ...
  it("README mentions approval-decision as second archetype Creator Workbench support", () => {
    expect(read("README.md")).toContain("approval-decision second archetype support");
    expect(read("README.zh-CN.md")).toContain("approval-decision 第二母型支持");
  });

  it("docs state approval-decision Studio UI is demo-enabled", () => {
    const combined = [
    ...
    expect(combined).toContain("Creator Workbench UI is demo-enabled");
  });
