...
- Added the demo/mock `diagnostic-resolution` fifth Product Archetype Core chain with an allowlisted generic-signal Pack Config, archetype-specific validation and Rule Impact metadata, deterministic six-artifact Rule Compiler, CLI compile/export, explicit diagnostic and remediation budgets, fail-closed inconclusive/retry/handoff/stop paths, and mock remediation fixed at `sideEffect=none`; Runtime, real diagnostics, device access, shell execution, external APIs, LLMs, and production remediation remain excluded.

### Added

- Enabled demo/mock `diagnostic-resolution` in Creator Workbench with a dedicated allowlisted-signal editor, Core Rule Impact, six-artifact Compile Preview, warnings and Certification Readiness, manual DSL Editor bridge, bilingual safety notice, and no automatic Inspect, Apply, Run, or Runtime access.
